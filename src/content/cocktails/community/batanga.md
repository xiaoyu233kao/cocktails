---
slug: batanga
nameZh: 巴坦加
nameEn: Batanga
category: Bar Assistant Community
baseSpirit: tequila
flavors:
  - fresh
styles:
  - built
  - highball
ingredients:
  - nameZh: 白龙舌兰
    nameEn: Tequila Blanco
    amount: 45 ml
    aliases:
      - Tequila Blanco
      - 白龙舌兰
  - nameZh: 青柠汁
    nameEn: Lime juice
    amount: 15 ml
    aliases:
      - Lime juice
      - 青柠汁
  - nameZh: 可乐
    nameEn: Cola
    amount: 1 topup
    aliases:
      - Cola
      - 可乐
tags:
  - community
  - bar-assistant
  - tequila
  - classics
  - fresh
  - built
  - highball
summary: 巴坦加以白龙舌兰、青柠汁、可乐为核心，采用直接调制调制，呈现清新的风味结构。
background: 巴坦加以白龙舌兰、青柠汁、可乐为核心，资料记录的基酒结构归入龙舌兰酒，并采用直接调制完成混合。成酒建议装入高球杯，以盐收尾；清新共同构成它的主要风味，甜酸、酒体和稀释度会随原料与冰量变化。建议先充分冷却，再在合适的冰量下尽快饮用，以保持香气、口感和温度的平衡。当前快照未提供可核验的创制年份或作者，因此不对年代、地点和人物作历史推断；原始来源与快照地址见来源字段。
historySources:
  - label: 原始来源 · Batanga
    url: https://imbibemagazine.com/recipe/la-batanga-from-ghost-donkey/
    checkedAt: 2026-09-20
  - label: Bar Assistant v5 snapshot · Batanga
    url: https://github.com/bar-assistant/data/blob/5a504d474614494119882eb91a8ffdc5491a483f/data/cocktails/batanga/data.json
    checkedAt: 2026-09-20
steps:
  - 将白龙舌兰、青柠汁、可乐按配方顺序加入高球杯，采用直接调制完成混合并加入适量冰块。
  - 以盐装饰后尽快饮用。
glass: Highball
garnish: 盐
image: /images/cocktails/batanga.svg
imageAlt: 巴坦加的程序化酒杯示意图
imageCredit:
  kind: generated
  creator: Cocktail Atlas procedural illustration
  source: local generator
  license: Project generated asset
  modified: true
  checkedAt: 2026-09-20
source:
  label: Bar Assistant v5 snapshot · Batanga
  url: https://github.com/bar-assistant/data/blob/5a504d474614494119882eb91a8ffdc5491a483f/data/cocktails/batanga/data.json
  checkedAt: 2026-09-20
variants: []
---
