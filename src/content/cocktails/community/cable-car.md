---
slug: cable-car
nameZh: 缆车
nameEn: Cable Car
category: Bar Assistant Community
baseSpirit: rum
flavors:
  - fresh
  - sweet
styles:
  - short
ingredients:
  - nameZh: 深色朗姆酒
    nameEn: Dark Rum
    amount: 45 ml
    aliases:
      - Dark Rum
      - 深色朗姆酒
  - nameZh: 君度
    nameEn: Cointreau
    amount: 22.5 ml
    aliases:
      - Cointreau
      - 君度
  - nameZh: 柠檬汁
    nameEn: Lemon juice
    amount: 30 ml
    aliases:
      - Lemon juice
      - 柠檬汁
  - nameZh: 糖浆
    nameEn: Simple Syrup
    amount: 15 ml
    aliases:
      - Simple Syrup
      - 糖浆
tags:
  - community
  - bar-assistant
  - rum
  - fresh
  - sweet
  - short
summary: 缆车以深色朗姆酒、君度、柠檬汁为核心，采用摇和调制，呈现清新、甜润的风味结构。
background: 缆车以深色朗姆酒、君度、柠檬汁为核心，资料记录的基酒结构归入朗姆酒，并采用摇和完成混合。成酒建议装入酒碟杯，以肉桂糖口收尾；清新、甜润共同构成它的主要风味，甜酸、酒体和稀释度会随原料与冰量变化。建议先充分冷却，再在合适的冰量下尽快饮用，以保持香气、口感和温度的平衡。当前快照未提供可核验的创制年份或作者，因此不对年代、地点和人物作历史推断；原始来源与快照地址见来源字段。
historySources:
  - label: 原始来源 · Cable Car
    url: https://www.drinkexistence.com/cable-car-cocktail/
    checkedAt: 2026-09-20
  - label: Bar Assistant v5 snapshot · Cable Car
    url: https://github.com/bar-assistant/data/blob/5a504d474614494119882eb91a8ffdc5491a483f/data/cocktails/cable-car/data.json
    checkedAt: 2026-09-20
steps:
  - 将深色朗姆酒、君度、柠檬汁、糖浆加入摇酒器，加入冰块后充分摇和至冰镇；滤入酒碟杯。
  - 以肉桂糖口装饰后尽快饮用。
glass: Coupe
garnish: 肉桂糖口
image: /images/cocktails/cable-car.svg
imageAlt: 缆车的程序化酒杯示意图
imageCredit:
  kind: generated
  creator: Cocktail Atlas procedural illustration
  source: local generator
  license: Project generated asset
  modified: true
  checkedAt: 2026-09-20
source:
  label: Bar Assistant v5 snapshot · Cable Car
  url: https://github.com/bar-assistant/data/blob/5a504d474614494119882eb91a8ffdc5491a483f/data/cocktails/cable-car/data.json
  checkedAt: 2026-09-20
variants: []
---
