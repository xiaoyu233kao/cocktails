---
slug: carajillo
nameZh: 卡拉希略
nameEn: Carajillo
category: Bar Assistant Community
baseSpirit: multi
flavors:
  - sweet
  - coffee
styles:
  - after-dinner
ingredients:
  - nameZh: 43号利口酒
    nameEn: Licor 43
    amount: 3 oz
    aliases:
      - Licor 43
      - 43号利口酒
  - nameZh: 浓缩咖啡
    nameEn: Espresso
    amount: 3 oz（cooled to room temperature or chilled）
    aliases:
      - Espresso
      - 浓缩咖啡
tags:
  - community
  - bar-assistant
  - multi
  - coffee
  - sweet
  - after-dinner
summary: 卡拉希略以43号利口酒、浓缩咖啡为核心，采用摇和调制，呈现甜润、咖啡的风味结构。
background: 卡拉希略以43号利口酒、浓缩咖啡为核心，资料记录的基酒结构归入复合基酒，并采用摇和完成混合。成酒建议装入古典杯，以橙皮扭饰收尾；甜润、咖啡共同构成它的主要风味，甜酸、酒体和稀释度会随原料与冰量变化。建议先充分冷却，再在合适的冰量下尽快饮用，以保持香气、口感和温度的平衡。当前快照未提供可核验的创制年份或作者，因此不对年代、地点和人物作历史推断；原始来源与快照地址见来源字段。
historySources:
  - label: 原始来源 · Carajillo
    url: https://imbibemagazine.com/recipe/carajillo/
    checkedAt: 2026-09-20
  - label: Bar Assistant v5 snapshot · Carajillo
    url: https://github.com/bar-assistant/data/blob/5a504d474614494119882eb91a8ffdc5491a483f/data/cocktails/carajillo/data.json
    checkedAt: 2026-09-20
steps:
  - 将43号利口酒、浓缩咖啡加入摇酒器，加入冰块后充分摇和至冰镇；滤入古典杯。
  - 以橙皮扭饰装饰后尽快饮用。
glass: Lowball
garnish: 橙皮扭饰
image: /images/cocktails/carajillo.webp
imageAlt: 卡拉希略的生成式酒杯示意图
imageCredit:
  kind: generated
  creator: OpenAI image generation
  source: OpenAI built-in image generation
  license: AI 生成示意图
  modified: true
  checkedAt: 2026-09-20
source:
  label: Bar Assistant v5 snapshot · Carajillo
  url: https://github.com/bar-assistant/data/blob/5a504d474614494119882eb91a8ffdc5491a483f/data/cocktails/carajillo/data.json
  checkedAt: 2026-09-20
variants: []
---
