---
slug: singapore-sling
nameZh: 新加坡司令
nameEn: Singapore Sling
category: Contemporary Classics
baseSpirit: gin
flavors: [tropical, fruity, citrus]
styles: [shaken, sling]
ingredients:
  - nameZh: 金酒
    nameEn: Gin
    amount: 30 ml
    aliases: [gin, London dry gin, 金酒]
  - nameZh: 樱桃白兰地
    nameEn: Cherry Sangue Morlacco
    amount: 15 ml
    aliases: [cherry liqueur, cherry brandy, 樱桃利口酒]
  - nameZh: 君度
    nameEn: Cointreau
    amount: 7.5 ml
    aliases: [cointreau, triple sec, 橙味利口酒]
  - nameZh: 本笃会甜酒
    nameEn: DOM Bénédictine
    amount: 7.5 ml
    aliases: [benedictine, herbal liqueur, 本笃会]
  - nameZh: 新鲜菠萝汁
    nameEn: Fresh Pineapple Juice
    amount: 120 ml
    aliases: [pineapple juice, fresh pineapple, 菠萝汁]
  - nameZh: 鲜青柠汁
    nameEn: Fresh Lime Juice
    amount: 15 ml
    aliases: [lime juice, fresh lime, 青柠汁]
  - nameZh: 石榴糖浆
    nameEn: Grenadine Syrup
    amount: 10 ml
    aliases: [grenadine, pomegranate syrup, 石榴糖浆]
  - nameZh: 安格式苦精
    nameEn: Angostura Bitters
    amount: 1 dash
    aliases: [angostura, bitters, 苦精]
tags: [gin, tropical, shaken, sling]
summary: 菠萝、樱桃与草本利口酒把金酒拉入一杯色彩明亮的热带长饮。
background: >-
  新加坡司令通常与 1915 年前后的莱佛士酒店长吧相连，酒店将海南籍调酒师 Ngiam Tong Boon 视为关键人物；但早期配方、创作者与现行版本之间存在断层，很多细节来自酒店后来的复原与传播，不能写成完全确定。它属于英属亚洲 sling 家族，以金酒、樱桃利口酒、草本与橙味利口酒、菠萝、青柠、石榴糖浆和苦精构成。20 世纪中叶后随酒店文化和旅游业全球化，成为热带长饮象征；现代 IBA 版本强调红粉色、菠萝香与飓风杯。
historySources:
  - label: "IBA Singapore Sling"
    url: "https://iba-world.com/iba-cocktail/singapore-sling/"
    checkedAt: 2026-09-19
  - label: "Raffles — How the Singapore Sling Conquered the World"
    url: "https://www.raffles.com/magazine/singapore/How-the-Singapore-Sling-conquered-the-world-/"
    checkedAt: 2026-09-19
steps:
  - 将所有原料与冰块加入摇酒器。
  - 摇匀后滤入飓风杯。
glass: Hurricane glass
garnish: 菠萝片与马拉斯奇诺樱桃
image: /images/cocktails/singapore-sling.webp
imageAlt: 饰以菠萝和樱桃的新加坡司令
imageCredit:
  kind: generated
  creator: OpenAI image generation
  source: OpenAI built-in image generation
  license: AI 生成示意图
  modified: true
  checkedAt: 2026-09-19
source:
  label: IBA Singapore Sling
  url: https://iba-world.com/iba-cocktail/singapore-sling/
  checkedAt: 2026-09-18
story:
  text: 莱佛士酒店把新加坡司令的创作归于 1915 年的海南籍调酒师 Ngiam Tong Boon；酒店也承认今天的配方与早期版本已经历多次调整，现有标准更像一条传播下来的家族谱系。
  source:
    label: Raffles — How the Singapore Sling Conquered the World
    url: https://www.raffles.com/magazine/singapore/How-the-Singapore-Sling-conquered-the-world-/
    checkedAt: 2026-09-18
variants:
  - name: Straits Sling
    description: 与新加坡司令同属英属亚洲 Sling 家族的相关变体，通常减少热带果汁并强化草本或苦味利口酒；这是非 IBA 版本。
    source:
      label: Oxford Companion to Spirits & Cocktails — Singapore Sling
      url: https://www.spiritsanddistilling.com/dictionary/acref-9780199311132-e-419
      checkedAt: 2026-09-18
---
