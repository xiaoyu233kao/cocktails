---
slug: south-side
nameZh: 南区
nameEn: South Side
category: New Era
baseSpirit: gin
flavors: [citrus, mint, sweet, herbal]
styles: [shaken, sour, up]
ingredients:
  - nameZh: 伦敦干金酒
    nameEn: London Dry Gin
    amount: 60 ml
    aliases: [gin, London dry gin, 金酒]
  - nameZh: 鲜柠檬汁
    nameEn: Fresh Lemon Juice
    amount: 30 ml
    aliases: [lemon juice, fresh lemon, 柠檬汁]
  - nameZh: 糖浆
    nameEn: Simple Syrup
    amount: 15 ml
    aliases: [simple syrup, sugar syrup, 糖浆]
  - nameZh: 薄荷叶
    nameEn: Mint Leaves
    amount: 5/6 leaves
    aliases: [mint, mint leaves, 薄荷]
  - nameZh: 蛋清（可选）
    nameEn: Egg White (Optional)
    amount: Few drops
    aliases: [egg white, egg, 蛋清]
tags: [gin, citrus, mint, sweet, herbal, shaken, sour, 南区]
summary: 金酒酸酒中加入薄荷，清凉、酸甜和杜松子香气在杯中交叠。
background: >-
  南区的早期记录和命名背景存在多种说法，常见叙述把它与20世纪初纽约南区酒吧、芝加哥或禁酒令时期的地下酒吧文化联系起来，但缺少能完全定案的单一来源。它的结构很接近带薄荷的 Gin Sour：伦敦干金酒、柠檬汁、糖浆和薄荷一起摇制，蛋清可选，用来增加柔滑泡沫。IBA 采用双重过滤后入冰镇鸡尾酒杯，并以薄荷枝装饰，让香气集中在入口和杯沿。与莫希托相比，南区不加入苏打水，酒体更紧致、酸度更清晰，也更适合短饮呈现。
historySources:
  - label: IBA South Side
    url: https://iba-world.com/iba-cocktail/south-side/
    checkedAt: 2026-09-19
  - label: Difford's Guide — South Side
    url: https://www.diffordsguide.com/cocktails/recipe/1914/south-side
    checkedAt: 2026-09-19
steps:
  - 将全部原料加入摇酒器，加冰充分摇匀并双重过滤入冰镇鸡尾酒杯。
  - 使用蛋清时需更加用力摇匀，以形成细腻泡沫。
glass: Chilled cocktail glass
garnish: 薄荷枝
image: /images/cocktails/south-side.webp
imageAlt: 以薄荷枝装饰的浅绿色南区鸡尾酒
imageCredit:
  kind: generated
  creator: OpenAI image generation
  source: OpenAI built-in image generation
  license: AI 生成示意图
  modified: true
  checkedAt: 2026-09-19
source:
  label: IBA South Side
  url: https://iba-world.com/iba-cocktail/south-side/
  checkedAt: 2026-09-19
variants:
  - name: Mojito
    description: 保留朗姆、薄荷、青柠和糖的清凉酸甜逻辑，并加入苏打水加冰长饮；南区则以金酒为基酒并保持短饮。
    source:
      label: IBA Mojito
      url: https://iba-world.com/iba-cocktail/mojito/
      checkedAt: 2026-09-19
---
