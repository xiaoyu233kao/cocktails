---
slug: spritz
nameZh: 斯普里茨
nameEn: Spritz
category: New Era
baseSpirit: wine
flavors: [bittersweet, sparkling, citrus]
styles: [built, aperitivo]
ingredients:
  - nameZh: 普罗塞克
    nameEn: Prosecco
    amount: 90 ml
    aliases: [prosecco, sparkling wine, 起泡酒]
  - nameZh: 阿佩罗
    nameEn: Aperol
    amount: 60 ml
    aliases: [aperol, bitter aperitif, 阿佩罗]
  - nameZh: 苏打水
    nameEn: Soda Water
    amount: splash
    aliases: [soda, sparkling water, 苏打]
tags: ["aperitif","bittersweet","built","aperitivo","Aperol Spritz"]
summary: 普罗塞克、阿佩罗和苏打水组成低负担、带橙香的起泡开胃酒。
background: >-
  Spritz 的名称通常被认为来自德语 spritzen（喷洒），19 世纪奥匈统治时期士兵把威尼托葡萄酒加水稀释的习惯是最常见的历史解释，但确切起点与统一配方并无单一结论。20 世纪后苦味利口酒和起泡酒逐渐加入，形成今日 aperitivo 结构；不同城市仍有 Select、Campari 等地方版本。IBA 以普罗塞克、阿佩罗和苏打水构成，葡萄酒杯加冰并以橙片收香。现代 Spritz 以低酒精感、气泡和橙香成为意大利开胃酒文化的国际符号。 关于Spritz的起源、传播与具体比例，现存记录可能因地区、时代和酒吧传统而有差异；本条以 IBA 页面及所列历史来源的可核对信息为准，并把后来的改写与当前标准配方分开说明。
historySources:
  - label: "IBA Spritz"
    url: "https://iba-world.com/iba-cocktail/spritz/"
    checkedAt: 2026-09-19
  - label: "Prosecco DOC — 50 Sfumature di Spritz"
    url: "https://www.prosecco.wine/50-sfumature-di-spritz/"
    checkedAt: 2026-09-19
steps:
  - 将所有原料依次加入装满冰块的葡萄酒杯。
  - 轻轻搅拌，使气泡和苦甜利口酒融合。
glass: Wine glass
garnish: 橙片
image: /images/cocktails/spritz.webp
imageAlt: 饰以橙片的阿佩罗斯普里茨
imageCredit:
  kind: generated
  creator: OpenAI image generation
  source: OpenAI built-in image generation
  license: AI 生成示意图
  modified: true
  checkedAt: 2026-09-19
source:
  label: IBA Spritz
  url: https://iba-world.com/iba-cocktail/spritz/
  checkedAt: 2026-09-18
story:
  text: Spritz 的名字通常与奥匈士兵把意大利葡萄酒加水稀释的习惯联系在一起，后来苦味利口酒和起泡酒加入其中，形成今日的 aperitivo 结构；不同地区的配方一直并存。
  source:
    label: Prosecco DOC — 50 Sfumature di Spritz
    url: https://www.prosecco.wine/50-sfumature-di-spritz/
    checkedAt: 2026-09-18
variants:
  - name: Campari Spritz
    description: 用 Campari 替代 Aperol，苦味和酒体更强；Prosecco DOC 将其列为地区化的非 IBA 版本。
    source:
      label: Prosecco DOC — 50 Sfumature di Spritz
      url: https://www.prosecco.wine/50-sfumature-di-spritz/
      checkedAt: 2026-09-18
  - name: Select Spritz
    description: 用 Select 替代 Aperol，带来更鲜明的草本苦味；这是威尼斯常见的非 IBA 版本。
    source:
      label: Prosecco DOC — 50 Sfumature di Spritz
      url: https://www.prosecco.wine/50-sfumature-di-spritz/
      checkedAt: 2026-09-18
---
